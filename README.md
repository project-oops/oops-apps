...
| **`mesa-probe`** | `src/oops-mesa/mesa-probe/` | OpenGL-through-Mesa bring-up app (`USE_MESA`; hosted rather than freestanding). Calls `radeonsi_screen_create` directly - the winsys path - and stays the control when `dri-probe` stops somewhere. |
| **`dri-probe`** | `src/oops-mesa/dri-probe/` | The same stack through the **Gallium DRI frontend** rather than the winsys directly: screen, drawable, context, a fixed-function triangle, a GLSL 330 one, and a full-frame hash of the result. Unit 6's gate (`0x5188ddb7`, oops-mesa worklog 066). A probe, so it answers and stops. |
