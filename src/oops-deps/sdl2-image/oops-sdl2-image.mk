# SDL2_image build integration. Include from a title's Makefile **after** `oops-sdl.mk` and
# `oops-libpng.mk`, before `common/app.mk`:
#
...
#
# It needs SDL2's headers and libpng's, and includes neither of those files itself - the same
# division `oops-sdl2-ttf.mk` draws next door, for the same reason: a title wanting images has
# already decided which SDL it uses.
#
# # PNG only
#
# `LOAD_PNG` is the one format switched on. Extreme Tux Racer, which is what wants this, calls
# `IMG_Load` at one call site against 149 PNG assets; its `.tga` and `.bmp` paths are its own
# screenshot *writers* and never arrive here.
#
...
# *not* to license.
#
# **`SDL_IMAGE_SAVE_PNG` is 0.** It defaults to 1 and would pull in libpng's writer for
# `IMG_SavePNG`, which nothing calls.
...
# libpng after it, because the loader calls into the decoder - `app.mk` puts LDFLAGS before the
# objects, and a static archive seen before its callers contributes nothing.
OOPS_IMG_LDFLAGS := $(OOPS_IMG_LIB) $(OOPS_PNG_LDFLAGS)
...
OOPS_IMG_CFLAGS = -target x86_64-unknown-freebsd -ffreestanding -fno-builtin -nostdlib \
                  ...
                  -DLOAD_PNG=1 -DSDL_IMAGE_SAVE_PNG=0 \
                  $(OOPS_IMG_INCLUDE) $(OOPS_PNG_INCLUDE) $(OOPS_SDL_INCLUDE) \
