 ...
 *
 * It lived under `include/freestanding/` at first, on the reasoning that a hosted title gets the
 * Mesa sysroot's real `xlocale.h` and so wants upstream's real backend. **The sysroot having the
 * header is not the same as the console having the function.** With the hosted libc++ built
 * against upstream's backend, the CTS's import manifest came back with a dozen names the
 ...
 *
 * Those are a *libc* API, and this platform's libc is the console's, not FreeBSD's. So the
 * choice of backend is about what the target exports, which is the same question in both
 * configurations - and this file belongs in `include/`, which both put on the path.
 *
 * `locale.h`, `nl_types.h` and `runetype.h` stay under `freestanding/`: those genuinely are
 * "does the header exist", and the sysroot's real ones are right for a hosted title.
 *
...
/*
 * Fuchsia's backend builds `std::locale::category` out of `LC_COLLATE_MASK` and its five
 * siblings. FreeBSD keeps those in `<xlocale.h>` rather than `<locale.h>`, and Fuchsia's file
 * includes only `<clocale>` - so on a hosted build they arrive undefined and every consumer of
 * `<locale>` stops on `use of undeclared identifier 'LC_MESSAGES_MASK'`.
 *
 * `include/freestanding/locale.h` declares them itself, so the freestanding build needs nothing
 * here. `__has_include` is what tells the two apart without a build-system flag: the sysroot has
...
/*
 * Fuchsia's backend also calls `::asprintf`, and FreeBSD declares that behind `__BSD_VISIBLE` -
 * which `_XOPEN_SOURCE` turns off, and `_XOPEN_SOURCE` is set for the reasons `gl-cts`'s
 * Makefile gives. So the declaration is made here rather than by relaxing visibility for every
 * translation unit, which would bring back the `u_long` collision it was set to avoid.
 */
#  include <stdio.h>
