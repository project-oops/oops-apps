 ...
 *
 * **oops-sdk declares this type too**, in `include/libc/wchar.h`, which it gained on 2026-09-23
 * for an unrelated reason: libc++'s `<cwchar>` needs `struct tm` to have been declared before
 * `<ctime>` gets to it, and C puts that declaration in `<wchar.h>`. The argument above - that a
 * `<wchar.h>` would bring functions with it - is why *this* file exists and still holds; the one
 * over there deliberately declares no functions at all.
 *
 * So two headers now define `mbstate_t` and a translation unit can see either first. They share
 * FreeBSD's own guard name rather than each keeping a private one, which makes the second
 * inclusion a no-op instead of a redefinition. **Both bodies must stay identical**: when they
 * were not, 21 of libc++'s 45 sources failed with `typedef redefinition with different types`
 * and nothing in the message named either file.
 */
