...
#
# The source list is upstream's own `OBJ`, not a glob: `uc.c`, `uc_argfn.c`, `uc_argtbl.c`,
# `uc_macrofn.c` and `uc_macrotbl.c` are not translation units - `uc.c:8-10` includes three of
# them and each `uc_f3d*.c` includes `uc.c` with a different microcode selected.
#
# Every microcode dialect is built because libultraship's display-list viewer lets the user
# choose one.
...
# Objects are named after their sources rather than numbered (see `common/deps.mk`).
