 ...
 *
 * Renders several controller states into a plain buffer and checks each drew and none wrote
 * past the surface. It does not check what the diagram says - on a host there is no pad - only
 * that the drawing is sound for neutral, fully-pressed, deflected, and touched states.
 */
...
int main(void) {
    ...
    state.pad.left_stick_x = 127;  state.pad.left_stick_y = -128;
    state.pad.right_stick_x = -128; state.pad.right_stick_y = 127;
    state.pad.touch[0].active = 1;  state.pad.touch[0].x = 0;    state.pad.touch[0].y = 0;
    state.pad.touch[1].active = 1;  state.pad.touch[1].x = 1919; state.pad.touch[1].y = 941;
    ...
    state.pad.left_stick_x = 40; state.pad.left_stick_y = -60;
    ...
    free(pixels);
    if (ok) {
        printf("pad-viz selftest: ok (states rendered; draw path and bounds verified)\n");
        return 0;
    }
    return 1;
}
