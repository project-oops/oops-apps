...
sample count so the low-latency read is visibly delivering more than one record per frame. That is
the path that preserves a press-and-release falling between two ordinary polls.
...
[D001](../../../docs/decisions/D001-a-repository-for-apps-built-on-the-sdk.md)): it shows the
controller, a person judges whether it matches what they are pressing, and there is no verdict.
Confirming the input record's field layout is obSCEne's job (the input census); this just draws
...
(`make skeleton`). The on-console payload — `pad-viz_main.c`, the display and the batched read
loop — links for the console with `make elf`.
