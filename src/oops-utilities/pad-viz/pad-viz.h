 ...
 *
 * Draws the DualSense/DualShock state as a stylised pad - buttons that light when
 * pressed, sticks whose thumbs move, analog trigger bars, the touch-pad with its
 * contacts, and a tilt box driven by the accelerometer. It is the on-screen counterpart
...
typedef struct padviz_state {
    oops_pad_state_t
        pad;          /* the controller sample to draw (the newest of a batched read) */
