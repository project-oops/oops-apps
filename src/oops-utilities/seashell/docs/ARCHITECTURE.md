...
The split that lets one codebase be both the Orbistoun front-end and a native console shell. The
...
2. **A native shell on real hardware** — ships as a native eboot Big App (category 0, root
   privilege), installed to `/user/app/SCSH00001`; a plain-ELF build is also produced for the
   `elfldr` / Orbistoun path.

That is possible because of the split above, which isolates UI state and software-canvas
...
Built under the strict warning set and shipped across the project-wide four axes
...
- `dist/seashell-title-prospero.zip` — full native Big App category 0 bundle ready to deploy to
  `/user/app/SCSH00001`.
