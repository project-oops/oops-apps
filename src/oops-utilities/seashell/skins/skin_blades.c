...
static int blades_render_main(oops_surface_t *surf, const struct home_model *m,
    ...
    if (act_blade < 0)
        act_blade = 0;
    if (act_blade >= skin->category_count)
        act_blade = skin->category_count - 1;
    ...
    oops_draw_rect_gradient(surf, bx, top_y, bw, blade_h, blade_bg_top, blade_bg_bot,
                            1);
    ...
    for (int j = 0; j < max_vis; j++) {
        ...
        char rating = 'G';
        home_get_category_item_info(m, skin, act_blade, idx, &name, &sub, &title_ptr,
                                    &rating);
        ...
        if (is_foc) {
            ...
            if (title_ptr != 0) {
                home_draw_title_icon(surf, title_ptr, lx + 8, iy + (ih - icon_sz) / 2,
                                     icon_sz, icon_sz, theme);
            } else {
                oops_draw_rect(surf, lx + 8, iy + (ih - icon_sz) / 2, icon_sz, icon_sz,
                               0xFF143D18u);
            }
            oops_draw_rect(surf, lx + 7, iy + ((ih - icon_sz) / 2) - 1, icon_sz + 2,
                           icon_sz + 2, 0xFFFFFFFFu);
    ...
    oops_draw_line_blend(surf, rx_card, ry_card, rx_card + rw_card, ry_card,
                         0x40FFFFFFu);
    ...
    char cur_rating = 'G';
    home_get_category_item_info(m, skin, act_blade, cur, &cur_name, &cur_sub,
                                &cur_title, &cur_rating);
    ...
    if (cur_title != 0) {
        ...
        home_draw_title_icon(surf, cur_title, art_x + (art_w / 2) - (icon_size / 2),
                             art_y + (art_h - icon_size) / 2, icon_size, icon_size,
                             theme);
    }
    ...
    (void)oops_draw_text(surf, art_x, desc_y,
                         "Experience high fidelity, ultra-responsive 60 FPS",
                         0xFFEFF7E8u, 1);
    desc_y += 16;
    (void)oops_draw_text(surf, art_x, desc_y,
    ...
    desc_y += 18;
    (void)oops_draw_text(surf, art_x, desc_y, "(>) 60 FPS Native Presentation",
                         0xFFB0BEC5u, 1);
    drawn += 10;
    ...
    (void)oops_draw_text(surf, leg_x + 180, leg_y + 2, "[SEARCH] Search", 0xFFEFF7E8u,
                         1);
...
const home_skin_t g_skin_blades = {
    ...
    .categories = {{"live", "network", HOME_CAT_CUSTOM, 0},
