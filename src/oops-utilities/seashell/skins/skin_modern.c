...
static int modern_render_main(oops_surface_t *surf, const struct home_model *m,
    ...
    /* Search */
    oops_color_t s_col =
        (m->top_nav == HOME_TOP_NAV_SEARCH) ? theme->accent : theme->text_dim;
    (void)oops_draw_text(surf, sw - 420, top_y + 2, "[SEARCH] SEARCH", s_col, 1);

    /* Settings [GEAR] */
    oops_color_t set_col =
        (m->top_nav == HOME_TOP_NAV_SETTINGS) ? theme->accent : theme->text_dim;
    ...
    for (int i = 0; i < count; i++) {
        ...
        char badge_code = ' ';
        home_get_category_item_info(m, skin, m->category_idx, i, &name, &sub, &title,
                                    &badge_code);
        ...
        int iw = 80;
        int ih = 80;
        if (iw > theme->tile_width - 16)
            iw = theme->tile_width - 16;
        if (ih > th - 36)
            ih = th - 36;
    ...
    if (cur >= 0 && cur < count) {
        ...
        char cur_badge = ' ';
        home_get_category_item_info(m, skin, m->category_idx, cur, &cur_name, &cur_sub,
                                    &cur_title, &cur_badge);
        ...
        char meta_line[128];
        oops_snprintf(
            meta_line, sizeof(meta_line), "[%s]  %s    VER: %s",
        ...
        (void)oops_draw_text(surf, theme->margin_x, dy, "[SELECT] PLAY", theme->text,
                             1);
        (void)oops_draw_text(surf, theme->margin_x + 120, dy, "[OPTIONS] OPTIONS",
                             theme->text_dim, 1);
        (void)oops_draw_text(surf, theme->margin_x + 280, dy, "[NAV] LIBRARY",
                             theme->text_dim, 1);
        drawn += 5;
    }
...
const home_skin_t g_skin_modern = {
    ...
    .categories = {{"games", "GAMES", HOME_CAT_GAMES, 0},
