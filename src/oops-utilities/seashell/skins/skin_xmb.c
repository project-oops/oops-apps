...
static void draw_xmb_category_icon(oops_surface_t *surf, int cat,
                                   int cx, int cy, int is_active, oops_color_t col) {
    int s = is_active ? 1 : 0;
    switch (cat) {
        case 0: /* USERS */
            oops_draw_circle_blend(surf, cx, cy - (s ? 8 : 6), s ? 7 : 5, col, 1);
            oops_draw_rect(surf, cx - (s ? 12 : 9), cy + (s ? 3 : 1), s ? 24 : 18, s ? 10 : 8, col);
            break;
        case 1: /* SETTINGS */
            oops_draw_rect(surf, cx - (s ? 12 : 9), cy - (s ? 4 : 3), s ? 24 : 18, s ? 16 : 12, col);
            oops_draw_rect(surf, cx - (s ? 5 : 4), cy - (s ? 9 : 7), s ? 10 : 8, 3, col);
            break;
        case 2: /* PHOTO */
            oops_draw_rect(surf, cx - (s ? 13 : 10), cy - (s ? 5 : 4), s ? 26 : 20, s ? 16 : 12, col);
            oops_draw_circle_blend(surf, cx, cy + (s ? 3 : 2), s ? 5 : 4, 0xFF000000u | (col & 0x00FFFFFFu), 0);
        ...
        case 3: /* MUSIC */
            ...
            break;
        case 4: /* VIDEO */
            oops_draw_rect(surf, cx - (s ? 12 : 9), cy - (s ? 8 : 6), s ? 24 : 18, s ? 18 : 14, col);
            oops_draw_rect(surf, cx - (s ? 7 : 5), cy - (s ? 3 : 2), s ? 14 : 10, s ? 8 : 6, 0xFF000000u);
            break;
        case 5: /* GAME */
            oops_draw_rect(surf, cx - (s ? 14 : 10), cy - (s ? 6 : 4), s ? 28 : 20, s ? 14 : 10, col);
            oops_draw_rect(surf, cx - (s ? 10 : 7), cy - (s ? 2 : 1), s ? 6 : 4, 2, 0xFF000000u);
            oops_draw_rect(surf, cx + (s ? 4 : 3), cy - (s ? 2 : 1), s ? 6 : 4, 2, 0xFF000000u);
            break;
        case 6: /* NETWORK */
        default:
            oops_draw_circle_blend(surf, cx, cy, s ? 9 : 7, 0xFF000000u | (col & 0x00FFFFFFu), 0);
...
static int xmb_render_main(oops_surface_t *surf, const struct home_model *m,
    ...
    for (int p = 0; p < XMB_PARTICLE_COUNT; p++) {
        ...
        if (s_particles[p].x >= sw) s_particles[p].x = 0;
        if (s_particles[p].y < 0) s_particles[p].y = sh - 1;
        if (s_particles[p].y >= sh) s_particles[p].y = 0;

        oops_draw_rect_blend(surf, s_particles[p].x, s_particles[p].y,
                             s_particles[p].size, s_particles[p].size, s_particles[p].color);
        drawn++;
    }
    ...
    int top_y = 36;
    int rx = sw - 320;
    (void)oops_draw_text(surf, rx, top_y, (m->status.user && m->status.user[0]) ? m->status.user : "USER 1",
                         theme->text, 1);
    int bat_x = rx + 110;
    ...
    oops_draw_line_blend(surf, 40, cat_row_y + 44, sw - 40, cat_row_y + 44, 0x20FFFFFFu);
    ...
    for (int i = 0; i < total_items; i++) {
        ...
        char badge_code = ' ';
        home_get_category_item_info(m, skin, active_cat, i, &name, &sub, &title, &badge_code);
        ...
        if (selected) {
            ...
            if (title != 0) {
                ...
                draw_xmb_category_icon(surf, active_cat, focus_cat_x - 3, iy + 21, 1, 0xFFFFFFFFu);
            }
            ...
            (void)oops_draw_text(surf, focus_cat_x + 32, iy + 4, disp_name, 0xFFFFFFFFu, 2);
            (void)oops_draw_text(surf, focus_cat_x + 32, iy + 26, sub, theme->accent, 1);
        } else {
            if (title != 0) {
                home_draw_title_icon(surf, title, focus_cat_x - 16, iy + 6, 30, 30, theme);
            } else {
                oops_draw_rect(surf, focus_cat_x - 16, iy + 6, 30, 30, theme->panel);
                draw_xmb_category_icon(surf, active_cat, focus_cat_x - 1, iy + 21, 0, theme->text_dim);
            }
            (void)oops_draw_text(surf, focus_cat_x + 32, iy + 12, name, theme->text_dim, 1);
        }
        drawn += 4;
    }
