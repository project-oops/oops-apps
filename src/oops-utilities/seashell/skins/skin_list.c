...
static int list_cursor_rect(const struct home_model *m, const struct home_skin *skin,
    ...
    (void)skin;
    if (m == 0 || x == 0 || y == 0 || w == 0 || h == 0) return 0;
    ...
    if (s_view == PS2_VIEW_MAIN) {
        ...
        *h = 36;
        ...
        if (total > 0 && cursor >= total) cursor = total - 1;
...
static int list_render_background(oops_surface_t *surf, const struct home_model *m,
    ...
    (void)m;
    (void)skin;
    if (surf == 0 || surf->pixels == 0) return 0;
...
static int list_render_main(oops_surface_t *surf, const struct home_model *m,
                            const struct home_skin *skin) {
    if (surf == 0 || surf->pixels == 0 || m == 0 || skin == 0) return 0;
    int drawn = 0;
    int sw = (int)surf->width;
    int sh = (int)surf->height;
    ...
    if (s_view == PS2_VIEW_MAIN) {
        ...
        oops_draw_circle_blend(surf, sphere_cx, sphere_cy, rx * 85 / 100, 0x0A001833u, 1);
        oops_draw_circle_blend(surf, sphere_cx, sphere_cy, rx * 65 / 100, 0x14002555u, 1);
        oops_draw_circle_blend(surf, sphere_cx, sphere_cy, rx * 45 / 100, 0x24004488u, 1);
        oops_draw_circle_blend(surf, sphere_cx, sphere_cy, rx * 30 / 100, 0x380077CCu, 1);
        oops_draw_circle_blend(surf, sphere_cx, sphere_cy, rx * 18 / 100, 0x5500AADDu, 1);
        oops_draw_circle_blend(surf, sphere_cx, sphere_cy, rx * 10 / 100, 0x7533DDFFu, 1);
        oops_draw_circle_blend(surf, sphere_cx, sphere_cy, rx * 5 / 100,  0xA5FFFFFFu, 1);
        drawn += 7;

        /* Swirling orbital light strands connecting the orbs */
        for (int i = 0; i < 7; i++) {
            int next = (i + 1) % 7;
            oops_draw_line_blend(surf, orbs[i].x, orbs[i].y, orbs[next].x, orbs[next].y, 0x220077BBu);
            drawn++;
        }
        ...
        /* Option 0: Browser */
        {
            ...
            if (s_main_cursor == 0) {
                oops_draw_rect_blend(surf, menu_x - 12, menu_y - 6, tw + 24, (text_scale * 8) + 12, 0x25006699u);
                oops_draw_rect_blend(surf, menu_x - 6, menu_y - 3, tw + 12, (text_scale * 8) + 6,  0x3800B4D8u);
                (void)oops_draw_text(surf, menu_x, menu_y, lbl, 0xFF00E5FFu, text_scale);
            } else {
                (void)oops_draw_text(surf, menu_x, menu_y, lbl, 0xFFD0D6E0u, text_scale);
            }
            drawn += 3;
        }
        ...
        /* Option 1: System Configuration */
        {
            ...
            if (s_main_cursor == 1) {
                oops_draw_rect_blend(surf, menu_x - 12, opt_y - 6, tw + 24, (text_scale * 8) + 12, 0x25006699u);
                oops_draw_rect_blend(surf, menu_x - 6, opt_y - 3, tw + 12, (text_scale * 8) + 6,  0x3800B4D8u);
        ...
        (void)oops_draw_text(surf, lx, by, "[SELECT] Enter    [OPTIONS] Options", 0xFFD0D6E0u, text_scale > 1 ? 2 : 1);
        drawn++;
        ...
        (void)oops_draw_text(surf, margin_x + 2, margin_y + 2, "Storage / 1", 0xFF000000u, 2);
            ...
            if (t->name && t->name[0] != '\0') {
                ...
                if (nlen > 18) {
                    ...
                    for (int k = 0; k < (int)nlen && k < 24; k++) {
                        if (t->name[k] == ' ') split_idx = k;
                    }
                    if (split_idx > 4) {
                        int p = 0;
                        for (p = 0; p < split_idx && p < (int)sizeof(line1) - 1; p++) line1[p] = t->name[p];
                        line1[p] = '\0';
                        int q = 0;
                        for (p = split_idx + 1; p < (int)nlen && q < (int)sizeof(line2) - 1; p++, q++) line2[q] = t->name[p];
                        line2[q] = '\0';
                    } else {
                ...
                (void)oops_draw_text(surf, right_x_base - w1 + 2, t_y + 2, line1, 0xFF000000u, 2);
                (void)oops_draw_text(surf, right_x_base - w1, t_y, line1, 0xFFFFDE59u, 2);
                drawn += 2;

                if (line2[0] != '\0') {
                    int w2 = oops_draw_text_width(line2, 2);
                    (void)oops_draw_text(surf, right_x_base - w2 + 2, t_y + 26 + 2, line2, 0xFF000000u, 2);
                    (void)oops_draw_text(surf, right_x_base - w2, t_y + 26, line2, 0xFFFFDE59u, 2);
                    drawn += 2;
                }
            }
        }
        ...
        if (total == 0) {
            (void)oops_draw_text(surf, sw / 2 - 80, sh / 2, "NO TITLES FOUND", 0xFF808894u, 2);
            drawn++;
        }
        ...
        for (int i = 0; i < total; i++) {
            ...
            /* Floor Drop Shadow */
            int shadow_base_y = start_y + r * (card_h + row_gap) + card_h + (sw > 1600 ? 14 : 10);
            int shadow_w = card_w * 7 / 10;
            int shadow_h = (sw > 1600 ? 14 : 10);
            int shadow_x = cx + (card_w - shadow_w) / 2;

            if (selected) {
                oops_draw_rect_blend(surf, shadow_x - 4, shadow_base_y, shadow_w + 8, shadow_h, 0x40000000u);
                oops_draw_rect_blend(surf, shadow_x, shadow_base_y + 2, shadow_w, shadow_h - 4, 0x55000000u);
            } else {
                oops_draw_rect_blend(surf, shadow_x, shadow_base_y, shadow_w, shadow_h, 0x55000000u);
                oops_draw_rect_blend(surf, shadow_x + 4, shadow_base_y + 2, shadow_w - 8, shadow_h - 4, 0x44000000u);
            }
            drawn += 2;
            ...
            if (selected) {
                ...
                oops_draw_circle_blend(surf, spot_cx, spot_cy, card_w * 90 / 100, 0x15FFFFFFu, 1);
                oops_draw_circle_blend(surf, spot_cx, spot_cy, card_w * 72 / 100, 0x28FFFFFFu, 1);
                oops_draw_circle_blend(surf, spot_cx, spot_cy, card_w * 54 / 100, 0x45FFFFFFu, 1);
                oops_draw_circle_blend(surf, spot_cx, spot_cy, card_w * 38 / 100, 0x70FFFFFFu, 1);
                oops_draw_circle_blend(surf, spot_cx, spot_cy, card_w * 24 / 100, 0xAAFFFFFFu, 1);
                oops_draw_circle_blend(surf, spot_cx, spot_cy, card_w * 14 / 100, 0xDDFFFFFFu, 1);
                drawn += 6;
            }

            /* Floating Card Bevel / Container */
            oops_draw_rect_blend(surf, cx + 4, cy + 4, card_w - 8, card_h - 8, 0x50181A20u);
            if (selected) {
                oops_draw_rect(surf, cx + 2, cy + 2, card_w - 4, card_h - 4, 0xFFFFFFFFu);
                oops_draw_rect(surf, cx + 3, cy + 3, card_w - 6, card_h - 6, 0x88A0E0FFu);
            } else {
                oops_draw_rect_blend(surf, cx + 3, cy + 3, card_w - 6, card_h - 6, 0x30FFFFFFu);
            }
            drawn += 2;

            /* Title Icon */
            const home_title_t *title = &m->titles[i];
            home_draw_title_icon(surf, title, cx + 8, cy + 8, card_w - 16, card_h - 16, theme);
            drawn++;
            ...
            if (m->switcher.has_running_title && m->switcher.running_title_index == i) {
                (void)oops_draw_text(surf, cx + 10, cy + card_h - 20, "[RUN]", 0xFF00FF88u, 1);
                drawn++;
            }
        }
