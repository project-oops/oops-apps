/*
 * Porthole - background daemon payload for low-latency streaming and remote input.
 *
 * Porthole runs in the background on the console (not as a foreground Big App).
 * Its purpose is to expose/tap into the system's unencrypted remote play stream
 * (via libSceVencCore / hardware video encoder) on TCP port 9805 and accept
 * DualSense controller input injection on TCP port 9806.
 *
            ...
            if (cap_rc == PORTHOLE_OK && frame_len > 0) {
                ...
                while (off < frame_len) {
                    ...
                    if (sent < 0 && porthole_would_block(sent)) {
                        if (++retries >= 500) {
                            /* Client is stalled or network fell too far behind (>500ms).
                             * Drop connection rather than starving the main loop and input port 9806. */
