 ...
 *
 * **What this is for.** oops-gl's GL 2.0 back end compiles a fragment shader to gfx1030
 * and the draw path binds it, and obSCEne has measured the pieces of that on hardware -
 * a generated shader retires (`REQ-...-4e77`), its parameters arrive, its uniform block
 * loads
 * (`REQ-...-6c0d`), and it can sample a texture (`REQ-...-2a45`). What none of those
 * measured is
 * **oops-gl's own draw path** putting a GL 2.0 program on screen: every one of them
 * bound a shader from obSCEne's fixture. This is the title that closes that gap, and
 * the first picture drawn by this compiler through `gl_draw.c`.
 *
 * It is deliberately the same cube as gl1-cube, in the same place, at the same angles.
 * The two titles then differ in exactly one thing - fixed function against a shader
 * program - so a difference in what they draw is a difference in that, and the
 * fixed-function half is already pinned to a measured console frame.
 *
 ...
 * signal. The same mechanism gl1-cube uses, and the reason it exists is recorded there.
 */
#define GL2_CUBE_STOP_PREFIX "/app0/stop."

static void cube_klog(const char *msg) {
    oops_klog("GL2-CUBE", msg);
}
#endif
...
/*
 * The model-view-projection matrix, built here rather than with `glLoadIdentity` and
 * friends.
 *
 * **A GL 2.0 program has no matrix stack.** `gl_ModelViewProjectionMatrix` exists for a
 * shader that wants the fixed-function state, and this deliberately does not use it: a
 * port written against GL 2.0 builds its own matrices and sends them as uniforms, and
 * that is the path this title is here to exercise. Column-major, which is what GL means
 * by a `mat4` and what `glUniformMatrix4fv` with `transpose` false expects.
 */
...
/* The same 45-degree vertical field of view, aspect and clip planes `gluPerspective`
 * gives gl1-cube, written out because there is no GLU on this path either. */
 ...
 *
 * **The log is printed, not swallowed.** A shader that fails to compile on a console
 * with no console is otherwise a black screen, and the whole reason
 * `glGetShaderInfoLog` exists is that the message names the line.
 */
...
__attribute__((visibility("default"))) int gl2_cube_start(const payload_args_t *args) {
    ...
    char stop_path[64];
    {
        ...
        int sp = 0;
        for (int i = 0; GL2_CUBE_STOP_PREFIX[i]; i++)
            stop_path[sp++] = GL2_CUBE_STOP_PREFIX[i];
     ...
     *
     * oops-gl gives a context the entry points its version names and no others, and the
     * default is 1.1 - so without this every `glCreateShader` below is
     * `GL_INVALID_OPERATION` and this title draws nothing at all. It is the first thing
     * after the context because everything after it depends on it.
     */
    ...
    /* **The shader objects are deleted now and the program keeps working.** That is
     * GL 2.0's deferred deletion, and doing it here is both what a real port does and a
     * live check of it: if the program stopped drawing after this, the reference
...
#ifndef OOPS_HOST_BUILD
    /* **What the console back end made of the fragment stage**, logged once. Zero words
     * means the compiler refused it - the draw will then fail with GL_INVALID_OPERATION
     * rather than drawing the fixed-function cube in its place, which is deliberate,
     * and this line is how a run says so before the first frame instead of after a
     * black one. */
    {
        GLint ps_words = 0;
    ...
    while (running) {
        ...
        /* projection * translate * rotX * rotY * rotZ, in that order, so the cube spins
         * about its own axes in front of the camera - the same composition gl1-cube's
         * matrix stack builds with `gluPerspective`, `glTranslatef` and three
         * `glRotatef`s. */
...
#ifndef OOPS_HOST_BUILD
        /* **The first draw's error, once.** A program the back end refused draws
         * nothing and records GL_INVALID_OPERATION; without this the run is a black
         * screen with no reason attached to it. */
        if (frame == 0u)
            cube_klog_hex("first-draw-error", (uint32_t)glGetError());
#endif
        glFinish();

        /* The render target's own numbers, the same three gl1-cube reports, so the two
         * titles' logs can be read side by side. One word in 64 unless this run is
         * reading the frame back, which is what the label says. */
...
#define OOPS_APP_VERSION "dev"
#endif
        /* Magenta rather than gl1-cube's cyan, so a screenshot says which of the pair
         * it is without anybody reading the text. */
        oops_draw_text(&surf, 60, 48, "OOPS-GL 2.0: SHADER CUBE (RDNA2)",
                       OOPS_COLOR_WHITE, 2);
        oops_draw_text(&surf, 60, 75, "GLSL compiled to gfx1030 by glsl_ps.c",
                       0xffff88ffu, 1);
        oops_draw_text(&surf, 60, 96, "Target: Prospero/Trinity (FW 12.40)",
                       0xffaaaaaau, 1);
        ...
        /* **The number this title exists to show.** How many gfx1030 instructions the
         * fragment shader became, and how many registers it wants. Zero words is a
         * refusal. */
        {
            ...
            int_to_str((int)ps_words, num_buf);
            sl = cube_append(stat_buf, sl, num_buf);
            sl = cube_append(stat_buf, sl, " words, ");
            int_to_str((int)ps_vgprs, num_buf);
            sl = cube_append(stat_buf, sl, num_buf);
...

        {
            ...
            sl = cube_append(stat_buf, sl, hx);
            sl = cube_append(stat_buf, sl,
                             full_scan ? "  |  Mod Pix: " : "  |  Mod Pix(1:64): ");
            int_to_str((int)mod_pixels, num_buf);
            sl = cube_append(stat_buf, sl, num_buf);
            sl = cube_append(stat_buf, sl, "  |  Center: ");
            hex_to_str(center_pix, hx);
            sl = cube_append(stat_buf, sl, hx);
        ...
        oops_snprintf(stat_buf, sizeof(stat_buf),
                      "Program: %u  |  Tris: 12  |  Verts: %u", (unsigned int)prog,
                      (unsigned int)GL2_CUBE_VERTEX_COUNT);
        ...
        if (frame == 5u || (frame % 60u) == 0u)
            cube_klog_hex("t-frame-us", last_frame_us);
#endif
        frame++;
    }
