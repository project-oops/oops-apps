/*
 * gl-replay - run a captured GL call stream against the software reference, on a build machine.
 *
 * `oops_gl_capture_frame` writes one of a title's frames to a file on the console. This reads
 * that file back and draws it here, where the rasteriser is the reference implementation and
 * where there is no console in the loop at all. The image it writes is what the frame is
 * *supposed* to look like; the difference from a screenshot of the same frame is the bug.
 *
 * **Why this exists.** A conformance suite tests what somebody thought to write down. Ninety-odd
 * checks in `gl1-probe` pass on hardware while a real port renders wrong, which is not a failure
 * of the suite - it is the suite reaching its edge, because the port is wrong in a combination
 * nobody wrote down. A capture is not written by anybody: it is what the program did.
 *
...
/* The same host display the other check-only apps use: a plain buffer, because oops-sdk's real
 * one talks to a backend that is not on a build machine. Full HD, so the row stride is the one
 * a captured frame was drawn against rather than a smaller lie. */
...
/* The build gate: capture a small drawing in process, replay it into a wiped buffer, and demand
 * the same pixels. It is the claim the whole tool rests on, so it runs on every build rather
 * than only when somebody remembers to point it at a file. */
static int selftest(void) {
    enum { W = 64, H = 64 };
    oops_display_t *disp = oops_display_open(OOPS_DISPLAY_BACKEND_AUTO, W, H);
    void *gc = glContextCreate(disp);
    if (!gc) { printf("gl-replay selftest: FAIL (no GL context)\n"); return 1; }
    glViewport(0, 0, W, H);
    ...
    if (!stream || !calls) { printf("gl-replay selftest: FAIL (nothing captured)\n"); return 1; }

    /* Magenta: neither the clear colour nor the triangle, so a replay that drew nothing shows. */
...
int main(int argc, char **argv) {
    ...
    if (!f) { fprintf(stderr, "gl-replay: cannot open %s\n", argv[1]); return 1; }
    fseek(f, 0, SEEK_END);
    long n = ftell(f);
    fseek(f, 0, SEEK_SET);
    if (n <= 0) { fprintf(stderr, "gl-replay: %s is empty\n", argv[1]); fclose(f); return 1; }
    ...
    /* The capture carries no window size - it is a call stream, not a frame buffer - so the
       replay is drawn at the size the console used, which is what its viewport calls expect. */
    oops_display_t *disp = oops_display_open(OOPS_DISPLAY_BACKEND_AUTO, 1920, 1080);
    void *gc = glContextCreate(disp);
    if (!gc) { fprintf(stderr, "gl-replay: no GL context\n"); free(buf); return 1; }
