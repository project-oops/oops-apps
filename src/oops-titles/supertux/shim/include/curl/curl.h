/*
 * libcurl, answered "there is no network" - the names SuperTux's add-on downloader uses
 * and no others. `../../curl_offline.cpp` defines them.
 *
 * `src/addon/downloader.cpp` includes this unconditionally (`HAVE_LIBCURL` is in
...
/* libcurl's values: `CURLOPTTYPE_LONG` is 0, `OBJECTPOINT` 10000, `FUNCTIONPOINT`
 * 20000. */
typedef enum {
