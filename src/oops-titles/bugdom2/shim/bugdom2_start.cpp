 ...
 * Data/Skeletons/Grasshopper.bg3d opens, so this probes for that file under each
 * candidate root and hands back the one that answered. The probe differs from Bugdom's
 * because this game brought its own model format.
 */
#include "oops/fs.h"
...
static int bug2_root_has_data(const char *root) {
    ...
    if (bug2_append(path, sizeof(path), base, "/Data/Skeletons/Grasshopper.bg3d") ==
        0u) {
        return 0;
    }
...
bugdom2_start(const payload_args_t *args) {
    ...
    /* Pomme's FindFolder reads XDG_CONFIG_HOME then HOME and returns fnfErr when both
       are unset, which upstream answers with an alert box. .config is the directory it
       appends, and it is called with kDontCreateFolder. */
