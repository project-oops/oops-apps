 ...
 *
 * The build ships soh.o2r, the port's own archive, and carries no game assets. oot.o2r and
 * oot-mq.o2r are generated on device from a ROM the player supplies, so a fresh install has
 * neither. libultraship answers that with SDL_ShowSimpleMessageBox ("Main OTR file not found"),
 * which on a console is a dialog with no pointer to dismiss it, so the same fact goes to the log
 * where it can be read.
 *
 * It reports rather than refuses: generating the archive is a thing the title itself does, so
 * reaching main with no archive and a ROM present is the normal first run.
 */
...
static void soh_report_game_data(void) {
    ...
    for (i = 0; i < sizeof(archives) / sizeof(archives[0]); i++) {
        if (oops_snprintf(path, sizeof(path), "%s/%s", OOPS_POSIX_HOME, archives[i]) > 0 &&
            oops_fs_exists(path)) {
    ...
    if (oops_snprintf(path, sizeof(path), "%s/%s", OOPS_POSIX_HOME, "soh.o2r") > 0 &&
        !oops_fs_exists(path)) {
        oops_log_error("SOH", "%s is missing: the port archive ships with the build", path);
    }
