...
### 3. Freestanding C Discipline (obscene#D008)
...
* **Dynamic Kernel Self-Resolution**: Dynamic import tables do not auto-bind in unsigned payload mode (`elfldr`). Porthole loads `VENC` via `sceSysmoduleLoadModule(0x00A0)` and walks the live kernel export table (`kproc + 0x3E8`) to resolve `libSceVencCore` entry points (obscene#D277 / obscene#D300).
...
| [`porthole.c`](porthole.c) | Core logic: freestanding `porthole_pad_decode`, dynamic encoder sysmodule load (`0x00A0`), kernel export table traversal (obscene#D277 / obscene#D300), and capture loops. | Both (target active, host stubs) |
...
* **M1 - Dynamic Load & Symbol Self-Resolution** : Completed (2026-09-03). Implemented dynamic loading of sysmodule `0x00A0`, live export table walk across `kproc + 0x3E8` (obscene#D277 / obscene#D300), self-resolution of `sceVencCore*` function pointers, and plain-ELF payload build (`build/porthole.elf`).
