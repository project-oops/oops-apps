...
static uint64_t test_virt_to_phys(uint64_t cr3, uint64_t dmap, uint64_t va) {
    ...
    uint64_t pml4e =
        mock_read64((uintptr_t)(dmap + (cr3 & 0x000FFFFFFFFFF000ULL) + pml4i * 8));
    if ((pml4e & 1ULL) == 0)
        return 0;

    uint64_t pdpte =
        mock_read64((uintptr_t)(dmap + (pml4e & 0x000FFFFFFFFFF000ULL) + pdpti * 8));
    if ((pdpte & 1ULL) == 0)
        return 0;
    if ((pdpte & 0x80ULL) != 0) {
    ...
    uint64_t pde =
        mock_read64((uintptr_t)(dmap + (pdpte & 0x000FFFFFFFFFF000ULL) + pdi * 8));
    if ((pde & 1ULL) == 0)
        return 0;
    if ((pde & 0x80ULL) != 0) {
    ...
    uint64_t pte =
        mock_read64((uintptr_t)(dmap + (pde & 0x000FFFFFFFFFF000ULL) + pti * 8));
    if ((pte & 1ULL) == 0)
        return 0;
...
int main(void) {
    ...
    /* Setup a 4KB mapping for va = 0x800C870C3 (PML4=16, PDPT=5, PD=2, PT=135,
     * offset=0x0C3) */
    ...
    pml4_page[pml4i] =
        ((uint64_t)(uintptr_t)pdpt_page) | 0x07ULL; /* Present + RW + User */
    ...
    printf("pltauth_patch_selftest: ok (31 ShellCore FW 12.40 patches verified, "
           "4-level page translation ok)\n");
    return 0;
}
