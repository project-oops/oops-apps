...
static uintptr_t sandbox_resolve_rootvnode(void) {
  ...
  if (proc1 != 0) {
    ...
    if (krw_copyout(proc1 + 0x48, &fd1, sizeof(fd1)) == 0 && fd1 != 0) {
      ...
      sandbox_log("PID 1 fd=0x%lx: cdir=0x%lx, rdir=0x%lx, jdir=0x%lx",
                  (unsigned long)fd1, (unsigned long)cdir1,
                  (unsigned long)rdir1, (unsigned long)jdir1);
      if (rdir1 != 0 && (rdir1 >> 40) != 0) return rdir1;
      if (cdir1 != 0 && (cdir1 >> 40) != 0) return cdir1;
    }
  }
  ...
  if (proc_self != 0) {
    uintptr_t fd_self = 0;
    if (krw_copyout(proc_self + 0x48, &fd_self, sizeof(fd_self)) == 0 && fd_self != 0) {
      ...
      if (rdir_self != 0 && (rdir_self >> 40) != 0) return rdir_self;
      if (cdir_self != 0 && (cdir_self >> 40) != 0) return cdir_self;
    }
  }
...
static int sandbox_elevate_process(pid_t target_pid, uintptr_t rootvnode) {
  ...
  if (rootvnode != 0) {
    ...
    if (rc_rdir != 0 || rc_jdir != 0) {
      ...
      sandbox_log("repointed fd_rdir, fd_jdir, and fd_cdir to rootvnode (0x%lx) for PID %d",
...
static void sandbox_handle_client(int client_fd, uintptr_t rootvnode) {
  ...
  if (result == 0) {
    ...
    sandbox_log("namespace elevation failed for PID %d (rc=%d)",
                (int)client_pid, result);
  }
}
...
#define OOPS_APP_VERSION "dev"
  ...
  sandbox_log("daemon starting v%s (args=%s)...", OOPS_APP_VERSION, args != NULL ? "present" : "null");
  ...
  if (test_dfd >= 0) {
    char test_buf[256];
    long basep554 = 0;
    long n554 = sys_call(554 /* SYS_getdirentries */, test_dfd, (long)test_buf, sizeof(test_buf), (long)&basep554, 0, 0);
    if (n554 > 0) {
      uint16_t r16 = *(uint16_t *)(test_buf + 16);
      uint16_t n20 = *(uint16_t *)(test_buf + 20);
      sandbox_log("SYS_getdirentries(554) SUCCESS: n=%ld, reclen@16=%u, namlen@20=%u, name@24='%s'",
                  n554, (unsigned int)r16, (unsigned int)n20, test_buf + 24);
      sandbox_log("  raw: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x",
                  (uint8_t)test_buf[0], (uint8_t)test_buf[1], (uint8_t)test_buf[2], (uint8_t)test_buf[3],
                  (uint8_t)test_buf[4], (uint8_t)test_buf[5], (uint8_t)test_buf[6], (uint8_t)test_buf[7],
                  (uint8_t)test_buf[8], (uint8_t)test_buf[9], (uint8_t)test_buf[10], (uint8_t)test_buf[11],
                  (uint8_t)test_buf[12], (uint8_t)test_buf[13], (uint8_t)test_buf[14], (uint8_t)test_buf[15]);
    } else {
      sandbox_log("SYS_getdirentries(554) returned %ld", n554);
    }

    long basep196 = 0;
    long n196 = sys_call(196 /* SYS_getdirentries_compat11 */, test_dfd, (long)test_buf, sizeof(test_buf), (long)&basep196, 0, 0);
    if (n196 > 0) {
      uint16_t r4 = *(uint16_t *)(test_buf + 4);
      uint8_t n7 = *(uint8_t *)(test_buf + 7);
      sandbox_log("SYS_getdirentries(196) SUCCESS: n=%ld, reclen@4=%u, namlen@7=%u, name@8='%s'",
  ...
  int sock = (int)sys_call(SYS_socket, SANDBOX_AF_INET,
  ...
  /* Listen */
  long listen_rc = sys_call(SYS_listen, sock, SANDBOX_LISTEN_BACKLOG, 0, 0, 0,
                            0);
  if (listen_rc != 0) {
  ...
  for (;;) {
    ...
    int client_fd =
        (int)sys_call(SYS_accept, sock, (long)client_addr, (long)&client_addr_len, 0, 0, 0);
    if (client_fd < 0) {
