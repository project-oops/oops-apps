...
Unlike daemons that continuously poll a trigger file, `sandbox-daemon` is fully event-driven:

- **Blocking socket** — listens on `127.0.0.1:9069` via a standard blocking accept.
- **Zero overhead** — while waiting, the daemon thread sleeps in the kernel's socket wait queue,
  consuming no CPU and doing no disk I/O.
- **On demand** — it wakes only when a client connects, services the handshake in microseconds,
  and returns to sleep.

## What it resolves dynamically
...
checking known processes against itself rather than baking in an address. The specific field
offsets it reads are defined and commented in the source.
