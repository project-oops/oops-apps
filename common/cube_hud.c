 ...
 *
 * Pure presentation: it reads an oops_cube_hud_t and draws it through oops/hud.h. It
 * formats with oops_snprintf (oops/freestd.h), which both a freestanding oops-gl title
 * and a hosted Mesa title link, so one renderer serves both.
 */
...
/* The dashboard palette, ARGB. Literals rather than <oops/draw.h>'s OOPS_COLOR_* so
 * this file depends on nothing but the overlay and the formatter. */
